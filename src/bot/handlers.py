...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(_(user_id, "start_welcome"))

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    await update.message.reply_text(_(user_id, "help_text"))
...
async def link_strava(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(_(user_id, "link_strava_prompt"), reply_markup=reply_markup)
...
async def toggle_strava_privacy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await update.message.reply_text(_(user_id, "privacy_not_linked"))
    else:
        ...
        conn.commit()
        if new_mode == 'public':
            await update.message.reply_text(_(user_id, "privacy_switched_public"))
        else:
            await update.message.reply_text(_(user_id, "privacy_switched_private"))
    conn.close()
...
async def get_last_activity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.message.chat.type != 'private':
            await update.message.reply_text(_(user_id, "last_activity_sent_privately"))
    except Exception as e:
        logger.error(f"为用户 {user_id} 获取最新活动时出错: {e}")
        await update.message.reply_text(_(user_id, "last_activity_error"))
    finally:
        conn.close()
...
async def get_last_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        placeholder_message = await update.message.reply_text(_(user_id, "fetching_video"))
...
async def get_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not current_stats or not current_stats[0]:
        await update.message.reply_text(_(user_id, "report_no_activity"), parse_mode='Markdown')
        return
...
async def get_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def format_leaders(title_key, leaders, unit):
        ...
        return "\n".join(lines)
    if not dist_leaders:
        await update.message.reply_text(_(user_id, "leaderboard_no_activity"))
        return
    message = [
...
async def my_achievements(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not unlocked_ids:
        await update.message.reply_text(_(user_id, "my_achievements_no_activity"))
        return
...
async def weather(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not context.args: await update.message.reply_text(_(user_id, "weather_prompt"))
...
async def route(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not context.args:
        await update.message.reply_text(_(user_id, "route_prompt"))
        return
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if not geo_data.get("results"):
                await update.message.reply_text(_(user_id, "route_location_not_found").format(location=location_name))
                return
            ...
            await update.message.reply_text(_(user_id, "route_error"))
...
async def location_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(_(user_id, "location_received"), reply_markup=reply_markup)
